...
- 语音模拟面试：AI 面试官逐轮提问，你用麦克风回答，结束后自动生成评分报告；内置三个岗位方向（机器人/IoT 整机测试、软件测试、AI/智能体测试），提问边界和真题库按岗位隔离
...
- 表达训练：一次一题的口述刻意练习，自动统计语速、语气词、口头禅、卡顿重复和结构信号，给出流畅度/结构化/笃定感三项分数、改写示范和连续打卡趋势
